    ...
    <table>
      <thead>
        <tr>
          ...
          <th>{{ t('balance_manager.number_of_deposits') }}</th>
          <th>{{ t('balance_manager.deposit_amount') }}</th>
      ...
      <tbody>
        ...
        <template v-else-if="dataDeposit && dataDeposit.length > 0">
          <template v-for="(item, index) in transformData(dataDeposit)" :key="index">
      ...
      <tfoot>
        <template v-if="dataDeposit && dataDeposit.length > 0">
          ...
          <template v-for="(item, index) in dataDeposit" v-else :key="index">
...
<script setup lang="ts">
  ...
  const totalNumber = computed(() => {
    const dataSum = transformData(props.dataDeposit)
    let total = 0
  ...
  const totalProfit = computed(() => {
    return formatCurrency.func(props.dataDeposit.reduce((acc: any, cur: any) => roundUp(acc) + roundUp(cur.profit), 0))
  })

  const totalPrevious = computed(() => {
    return formatCurrency.func(
      props.dataDeposit.reduce((acc: any, cur: any) => roundUp(acc) + roundUp(cur.previous_month), 0),
    )
  })

  const checkAdjustment = computed(() => {
    const findIndex = props.dataDeposit.findIndex((item) => item.is_manual === 1 && !item.client_id)
    if (findIndex !== -1) {
      return true
    }
    return false
  ...
  watch(
    () => props.dataDeposit,
    () => {
      props.dataDeposit.forEach((item) => {
  ...
  const getDataSubmit = () => {
    if (checkAdjustment.value || !props.dataDeposit || props.dataDeposit.length === 0) {
      return
    } else {
      const dataSubmit = {
        profit: profit.value,
        memo: memo.value,
        income_expenditure_id: props.dataDeposit[0].income_expenditure_id,
        type: props.dataDeposit[0].type,
        is_manual: 1,
      }
      return dataSubmit
    }
