<template>
  <va-card class="flex">
    <va-card-content class="c-pb-50">
      <div class="title c-mb-36">{{ t('bank.bank_edit_title') }}</div>
...
<script setup lang="ts" scoped>
  ...
  import { FormUpdateBank } from '../../../types/Bank'
  ...
  const data = reactive(<FormUpdateBank>{})
  ...
  const getBankById = async () => {
    try {
      const result = await BankService.getBankById(route.params.id)
        .then((res) =>{
          return res.data
        })
      Object.assign(data, result)
      data.client_withdrawal_fee_1 = Number(data.client_withdrawal_fee_1).toLocaleString("ja-JP")
      data.client_withdrawal_fee_2 = Number(data.client_withdrawal_fee_2).toLocaleString("ja-JP")
      data.contract_withdrawal_fee_1 = Number(data.contract_withdrawal_fee_1).toLocaleString("ja-JP")
      data.contract_withdrawal_fee_2 = Number(data.contract_withdrawal_fee_2).toLocaleString("ja-JP")
      data.difference_fee = Number(data.difference_fee).toLocaleString("ja-JP")
      if(data.bank_list_name) {
        arrayBankListName.value = data.bank_list_name.split(',')
      }
    } catch(error) {
      console.log(error)
    }
  ...
  const onEditClick = async () => {
    ...
    const result = await service.update(`${API.MANAGEMENT_COMMON.BANK.UPDATE(route.params.id)}`, params)
    if (result.status) {
      GlobalStore.setLoading(false)
