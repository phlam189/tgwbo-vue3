<template>
  <va-card class="flex">
    <va-card-content class="c-pb-50">
      <div class="title c-mb-36">{{ t('expense.expense_add_title') }}</div>
      <div class="row va-gutter-1">
          <div class="flex md4 lg2 sm6 xs12 form-label align-self-center">
            <div>{{ t('expense.expense_name') }}
              <span class="mark-required"> *</span></div>
          </div>
...
<script setup lang="ts">
  ...
  import { required, helpers, numeric, maxLength, decimal } from '@vuelidate/validators'
  ...
  const onAddClick = async () => {
    ...
    const params = {...data}
    const result = await service.create(`${API.MANAGEMENT_COMMON.EXPENSE.CREATE}`, params)
    if (result.status) {
      GlobalStore.setLoading(false)
