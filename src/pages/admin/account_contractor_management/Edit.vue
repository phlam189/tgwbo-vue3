<template>
  <va-card class="flex">
    <va-card-content class="c-pb-50">
      <div class="title c-mb-36">{{ t('contractor.account_contractor_edit_title') }}</div>
...
<script setup lang="ts">
  ...
  import { required, helpers, integer, minLength, maxLength, email } from '@vuelidate/validators'
  ...
  import { FormUpdateAccountContractor } from '../../../types/Contractor'
  ...
  const data = reactive(<FormUpdateAccountContractor>{})
  ...
  const getContractorById = async () => {
    try {
      const result = await ContractorService.getContractorById(route.params.id)
        .then((res) =>{
          return res.data
        })
      Object.assign(data, result)
      data.presence = {
        text: data.presence == 1 ? t('contractor.option_have') : t('contractor.option_none'),
        value: data.presence
      }
      data.existence = {
        text: data.existence == 1 ? t('contractor.option_have') : t('contractor.option_none'),
        value: data.existence
      }
      var text_company_type = ''
      switch (data.company_type) {
        case COMPANY_TYPE.FORMER_STOCK:
          text_company_type = t('contractor.former_stock')
          break
        case COMPANY_TYPE.BACK_STOCK:
          text_company_type = t('contractor.back_stock')
          break
        case COMPANY_TYPE.FRONT:
          text_company_type = t('contractor.front')
          break
        case COMPANY_TYPE.BACK:
          text_company_type = t('contractor.back')
          break
        case COMPANY_TYPE.PREEXISTING:
          text_company_type = t('contractor.preexisting')
          break
        case COMPANY_TYPE.LATER:
          text_company_type = t('contractor.later')
          break
        default:
          text_company_type = t('contractor.none')
      }
      data.company_type = {
  ...
  const isUniqueEmail = async (email: any) => {
    try {
      if(!email) return
      const response = await ContractorService.checkUniqueEmail(email, route.params.id);
  ...
  const onEditClick = async () => {
    ...
    const result = await service.update(`${API.MANAGEMENT_COMMON.CONTRACTOR.UPDATE(route.params.id)}`, params)
    if (result.status) {
      GlobalStore.setLoading(false)
