<template>
  <va-card class="flex">
    <va-card-content>
      <div class="title c-mb-36">{{ t('client.client_edit_title') }}</div>
      ...
      <div class="d-flex flex gap-35 c-mt-15">
        <div class="flex flex-col md6">
          <p class="lable-input">{{ t('client.contract_use_service') }}</p>
          <va-input v-model="data.contract_use_service" disabled/>
        </div>
        <div class="flex flex-col md6">
        </div>
      </div>
      ...
      :client_id="client_id"
      :id="route.params.id"
      :contract_method="data.contract_method?.value"
      />
...
<script setup lang="ts">
  ...
  import { required, helpers, integer, numeric, maxLength, email } from '@vuelidate/validators'
  import { ref, computed, reactive, toRefs, onMounted, watch } from 'vue'
  import { useRouter, useRoute } from 'vue-router'
  import { useToast } from 'vuestic-ui'
  import { useGlobalStore } from '../../../stores/global-store'
  import { FormUpdateClient } from '../../../types/Client'
  ...
  import ClientDetail from '../../admin/client-detail-management/List.vue'
  ...
  const data = reactive(<FormUpdateClient>{})
  ...
  const getClientById = async () => {
    try {
      const result = await ClientService.getClientById(route.params.id)
        .then((res) =>{
          return res.data
        })
      ...
      if(data.presence || data.presence === 0) {
        data.presence = {
          text: data.presence == 1 ? t('client.option_have') : t('client.option_none'),
          value: data.presence
        }
      }
      if(data.contract_method || data.contract_method === 0) {
        data.contract_method = {
          text: data.contract_method == 1 ? t('client.slide') : t('client.flat'),
          value: data.contract_method
        }
      }
      if(data.termination_date) {
        let date = (new Date(data.termination_date))
        data.termination_date = date
      } else {
        data.termination_date = null
      }
      data.is_transfer_fee = {
        text: data.is_transfer_fee == TRANSFER_FEE_TYPE.CAN_BE ? t('client.can_be') : t('client.none'),
        value: data.is_transfer_fee
      }
    } catch(error) {
      console.log(error)
    }
  ...
  const isUniqueEmail = async (email: any) => {
    try {
      if(!email) return
      const response = await ClientService.checkUniqueEmail(email, route.params.id);
  ...
  const isUniqueClientId = async (client_id: any, id: any) => {
    try {
      const response = await ClientService.checkUniqueClientId(client_id, id);
      return response.data === 1;
    } catch (error) {
      console.log(error)
    }
  ...
  const onEditClick = async () => {
    ...
    var isExistClient = await isUniqueClientId(data.client_id, route.params.id)
    ...
    const result = await service.update(`${API.MANAGEMENT_COMMON.CLIENT.UPDATE(route.params.id)}`, params)
    if (result.status) {
      GlobalStore.setLoading(false)
